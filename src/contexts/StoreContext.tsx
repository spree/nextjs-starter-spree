...
export function StoreProvider({ children, initialCountry, initialLocale }: StoreProviderProps) {
      ...
      try {
        ...
        if (!urlCountryInList && countriesData.data.length > 0) {
          ...
          const defaultCountry = defaultCountryIso
            ? countriesData.data.find((c) => c.iso.toLowerCase() === defaultCountryIso)
            : countriesData.data[0]

          if (defaultCountry) {
            const newLocale = defaultCountry.default_locale || storeData.default_locale || 'en'
            ...
            setCurrency(defaultCountry.default_currency || storeData.default_currency)
